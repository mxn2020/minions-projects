...
description: Agent skills for working with Minions Projects MinionTypes. Provides CRUD operations, CLI usage, and best practices for AI agents managing minions-projects data.
---

# Minions Projects Agent Skills

Skills for agents operating on the `minions-projects` toolbox.
...
The `projects` CLI provides basic project info and utilities:
...
```ts
import { customTypes } from '@minions-projects/sdk/schemas';
...
```python
from minions_projects.schemas import custom_types
...
- This agent only writes to `minions-projects` — it reads from other toolboxes but never writes to them
